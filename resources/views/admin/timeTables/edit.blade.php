...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.time-tables.update", [$timeTable->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.timeTable.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $timeTable->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('program') ? 'is-invalid' : '' }}" name="program_id" id="program_id" required>
                    @foreach($programs as $id => $program)
                        <option value="{{ $id }}" {{ (old('program_id') ? old('program_id') : $timeTable->program->id ?? '') == $id ? 'selected' : '' }}>{{ $program }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('semester') ? 'is-invalid' : '' }}" type="number" name="semester" id="semester" value="{{ old('semester', $timeTable->semester) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('year') ? 'is-invalid' : '' }}" type="text" name="year" id="year" value="{{ old('year', $timeTable->year) }}">
                @if($errors->has('year'))
                    <div class="invalid-feedback">
                        {{ $errors->first('year') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('exam_available_days') ? 'is-invalid' : '' }}" name="exam_available_days_id" id="exam_available_days_id">
                    @foreach($exam_available_days as $id => $exam_available_days)
                        <option value="{{ $id }}" {{ (old('exam_available_days_id') ? old('exam_available_days_id') : $timeTable->exam_available_days->id ?? '') == $id ? 'selected' : '' }}>{{ $exam_available_days }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('start_time') ? 'is-invalid' : '' }}" type="text" name="start_time" id="start_time" value="{{ old('start_time', $timeTable->start_time) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('academic_year') ? 'is-invalid' : '' }}" name="academic_year_id" id="academic_year_id">
                    @foreach($academic_years as $id => $academic_year)
                        <option value="{{ $id }}" {{ (old('academic_year_id') ? old('academic_year_id') : $timeTable->academic_year->id ?? '') == $id ? 'selected' : '' }}>{{ $academic_year }}</option>
                    @endforeach
                </select>
