...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.electiveGroup.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.elective-groups.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('programs') ? 'is-invalid' : '' }}" name="programs[]" id="programs" multiple>
                    @foreach($programs as $id => $programs)
                        <option value="{{ $id }}" {{ in_array($id, old('programs', [])) ? 'selected' : '' }}>{{ $programs }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('offered_subjects_in_group') ? 'is-invalid' : '' }}" type="number" name="offered_subjects_in_group" id="offered_subjects_in_group" value="{{ old('offered_subjects_in_group', '') }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('max_subjects_allowed') ? 'is-invalid' : '' }}" type="number" name="max_subjects_allowed" id="max_subjects_allowed" value="{{ old('max_subjects_allowed', '') }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('min_subjects_required') ? 'is-invalid' : '' }}" type="number" name="min_subjects_required" id="min_subjects_required" value="{{ old('min_subjects_required', '') }}" step="1">
