    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.programs.update", [$program->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <label class="required" for="title">{{ trans('cruds.program.fields.title') }}</label>
                            <input class="form-control" type="text" name="title" id="title" value="{{ old('title', $program->title) }}" required>
                            @if($errors->has('title'))
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="code" id="code" value="{{ old('code', $program->code) }}" required>
                            @if($errors->has('code'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('code') }}
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="category" id="category" value="{{ old('category', $program->category) }}">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="faculty_id" id="faculty_id">
                                @foreach($faculties as $id => $faculty)
                                    <option value="{{ $id }}" {{ (old('faculty_id') ? old('faculty_id') : $program->faculty->id ?? '') == $id ? 'selected' : '' }}>{{ $faculty }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="colleges[]" id="colleges" multiple>
                                @foreach($colleges as $id => $college)
                                    <option value="{{ $id }}" {{ (in_array($id, old('colleges', [])) || $program->colleges->contains($id)) ? 'selected' : '' }}>{{ $college }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="level_id" id="level_id">
                                @foreach($levels as $id => $level)
                                    <option value="{{ $id }}" {{ (old('level_id') ? old('level_id') : $program->level->id ?? '') == $id ? 'selected' : '' }}>{{ $level }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('level'))
                                <div class="invalid-feedback">
